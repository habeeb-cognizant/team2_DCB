...
$(document).ready(function () {
    ...
    if ($('#signupForm').length) {
        $('#signupForm').on('submit', function (e) {
            ...
            // Custom validations
            if (!isValidEmail(emailInput.val())) {
                emailInput.get(0).setCustomValidity("Invalid email format.");
            } else if (AppState.users.find(user => user.email === emailInput.val())) {
                 emailInput.get(0).setCustomValidity("This email is already registered.");
            } else {
                emailInput.get(0).setCustomValidity("");
            }

            if (!isValidPassword(passwordInput.val())) {
                passwordInput.get(0).setCustomValidity("Invalid password format.");
            } else {
                passwordInput.get(0).setCustomValidity("");
            }

            if (passwordInput.val() !== confirmPasswordInput.val()) {
                confirmPasswordInput.get(0).setCustomValidity("Passwords do not match.");
            } else {
                confirmPasswordInput.get(0).setCustomValidity("");
            }
    ...
    if ($('#loginForm').length) {
        $('#loginForm').on('submit', function (e) {
            ...
            setTimeout(() => {
                // Find user in our local storage 'database'
                const foundUser = AppState.users.find(user => user.email === email && user.password === password);
    ...
    // 4. LOGOUT (Works for both User and Admin)
    $('#logout').on('click', function (e) {
        e.preventDefault();
        sessionStorage.removeItem('loggedInEmail');
        sessionStorage.removeItem('isAdminLoggedIn');
